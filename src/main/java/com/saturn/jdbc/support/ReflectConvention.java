...
public class ReflectConvention {
    ...
    private static final Logger log = LoggerFactory.getLogger(ReflectConvention.class);

    private static final DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:ss:mm");
    ...
    private static Object _appendSBByField(Object data, Field field, StringBuilder tmp, String condition, String valueTemplate, boolean useQuestionmark, boolean useAnd) {
        ...
        try {
            ...
            try {
                fieldValueObj = ReflectConventionUtil.getValue(data, field);
            } catch (Exception e) {
            ...
            if (!useQuestionmark && date)
                fieldValueObj = formatter.format(LocalDateTime.ofInstant(((Date) fieldValueObj).toInstant(), ZoneId.systemDefault()));// 所有时间相关类型都先转变成默认格式
    ...
    public static Object appendValuesByField(Object data, Field field, StringBuilder tmp, StringBuilder tmp2, boolean useQuestionmark) {
        try {
            ...
            boolean addQuote = field.getType() == String.class || date;
            if (!useQuestionmark && date)
                formatter.format(LocalDateTime.ofInstant(((Date) fieldValueObj).toInstant(), ZoneId.systemDefault())); // 所有时间相关类型都先转变成默认格式
